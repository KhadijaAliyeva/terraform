    ...
    stage('Terraform Apply') {
      ...
      steps {
        withCredentials([string(credentialsId: 'aws-credentials', variable: 'AWS_CREDENTIALS')]) {
          sh 'echo "$AWS_CREDENTIALS" > /tmp/aws-credentials'
          withEnv(['AWS_SHARED_CREDENTIALS_FILE=/tmp/aws-credentials']) {
            sh "terraform apply -auto-approve -var 'region=${params.aws_region}' -var 'instance_name=${params.instance_name}' -var 'ami=${var.ami}' -var 'instance_type=${var.instance_type}' -var 'key_name=${var.key_name}' -var 'subnet_id=${var.subnet_id}'"
          }
        }
      }
    }
    ...
    stage('Terraform Destroy') {
      ...
      steps {
        withCredentials([string(credentialsId: 'aws-credentials', variable: 'AWS_CREDENTIALS')]) {
          sh 'echo "$AWS_CREDENTIALS" > /tmp/aws-credentials'
          withEnv(['AWS_SHARED_CREDENTIALS_FILE=/tmp/aws-credentials']) {
            sh "terraform destroy -auto-approve -var 'region=${params.aws_region}' -var 'instance_name=${params.instance_name}'"
          }
        }
      }
    }
